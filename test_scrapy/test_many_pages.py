...
def find_links(soup, base_url):
    ...
    for link in soup.find_all('a', href=True):
        href = link['href']
        if href.startswith('/wiki/') and not href.startswith(("/wiki/Wikipedia:", "/wiki/%E7%89%B9%E5%88%A5:", "/wiki/Help:", "/wiki/File:")) and not '.svg' in href and not '.jpeg' in href and not '.jpg' in href:
            links.append(base_url + href)
    return links
...
start_url = 'https://ja.wikipedia.org/wiki/%E4%B8%BB%E6%88%90%E5%88%86%E5%88%86%E6%9E%90'
...
# 結果をCSVに保存
df.to_csv('output_PCA.csv', index=False, quoting=csv.QUOTE_NONNUMERIC, encoding='utf-8-sig')
